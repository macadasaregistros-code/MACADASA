...
import { transformFinance, type TransformFinanceSummary } from "../src/transform/finance";
...
async function startTransformRun(dryRun: boolean): Promise<string | null> {
  ...
  const { data, error } = await supabase
    .from("transform_runs")
    .insert({
      transform_name: "finance",
      mode: "live",
      status: "running",
      metadata: {
        started_by: "scripts/transform-finance.ts"
      }
    })
    .select("id")
    .single();
...
async function finishTransformRun(
  ...
  summary: TransformFinanceSummary | null,
  errorMessage?: string
): Promise<void> {
  if (!runId) {
    return;
...
function printSummary(summary: TransformFinanceSummary): void {
  console.log("\nTransform finance summary");
...
async function main(): Promise<void> {
  ...
  let summary: TransformFinanceSummary | null = null;

  try {
    console.log(options.dryRun ? "Starting finance transform in dry-run mode." : "Starting finance transform.");
    if (options.only && options.only.size > 0) {
      console.log(`Stages filter: ${[...options.only].join(", ")}`);
    }

    summary = await transformFinance(supabase, {
