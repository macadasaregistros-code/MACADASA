...
import {
  transformFeedPlant,
  type TransformFeedPlantSummary
} from "../src/transform/feedPlant";
...
async function startTransformRun(dryRun: boolean): Promise<string | null> {
  ...
  const { data, error } = await supabase
    .from("transform_runs")
    .insert({
      transform_name: "feed_plant",
      mode: "live",
      status: "running",
      metadata: {
        started_by: "scripts/transform-feed-plant.ts"
      }
    })
    .select("id")
    .single();
...
async function finishTransformRun(
  ...
  summary: TransformFeedPlantSummary | null,
  errorMessage?: string
): Promise<void> {
  if (!runId) {
    return;
...
function printSummary(summary: TransformFeedPlantSummary): void {
  console.log("\nTransform feed plant summary");
...
async function main(): Promise<void> {
  ...
  let summary: TransformFeedPlantSummary | null = null;

  try {
    console.log(
      options.dryRun ? "Starting feed plant transform in dry-run mode." : "Starting feed plant transform."
    );
    if (options.only && options.only.size > 0) {
      console.log(`Stages filter: ${[...options.only].join(", ")}`);
    }

    summary = await transformFeedPlant(supabase, {
