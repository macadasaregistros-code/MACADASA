...
import { transformInventory, type TransformInventorySummary } from "../src/transform/inventory";
...
async function startTransformRun(dryRun: boolean): Promise<string | null> {
  ...
  const { data, error } = await supabase
    .from("transform_runs")
    .insert({
      transform_name: "inventory",
      mode: "live",
      status: "running",
      metadata: {
        started_by: "scripts/transform-inventory.ts"
      }
    })
    .select("id")
    .single();
...
async function finishTransformRun(
  ...
  summary: TransformInventorySummary | null,
  errorMessage?: string
): Promise<void> {
  if (!runId) {
    return;
...
function printSummary(summary: TransformInventorySummary): void {
  console.log("\nTransform inventory summary");
...
async function main(): Promise<void> {
  ...
  let summary: TransformInventorySummary | null = null;

  try {
    console.log(options.dryRun ? "Starting inventory transform in dry-run mode." : "Starting inventory transform.");
    if (options.only && options.only.size > 0) {
      console.log(`Stages filter: ${[...options.only].join(", ")}`);
    }

    summary = await transformInventory(supabase, {
